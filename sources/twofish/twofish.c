...
static unsigned int
f32c(unsigned int x, unsigned int * k32, int keyLen)
{
...
/* Most of the C in this function was "written" by critcl when
   ...
   address of the array on to f32c function.
*/
static int F32c_Cmd(ClientData cdata, Tcl_Interp *interp, int objc, Tcl_Obj *const objv[])
{
    Tcl_WideInt temp;
 ...
 * Adapted from: http://wiki.tcl.tk/11153
 */
int DLLEXPORT F32c_Init(Tcl_Interp *interp)
{
    if (Tcl_InitStubs(interp, TCL_VERSION, 0) == NULL) {
        return TCL_ERROR;
    }

    if (Tcl_PkgProvide(interp, "itwofish", "0.2") == TCL_ERROR) {
        return TCL_ERROR;
    }

    Tcl_CreateObjCommand(interp, "f32c", F32c_Cmd, NULL, NULL);
    return TCL_OK;
}
